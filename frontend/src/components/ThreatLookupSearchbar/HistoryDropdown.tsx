...
export function HistoryDropdown({
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'ArrowDown') {
        ...
        event.preventDefault()
        onSelect(filteredHistory[highlightedIndex].ip)
        onClose()
  ...
  }, [isOpen, highlightedIndex, filteredHistory, onSelect, onClose])
          ...
          {filteredHistory.map((entry, index) => (
            ...
            >
              <button
                ...
                className="flex items-center gap-3 text-left hover:text-blue-600 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 rounded px-1 py-1"
              >
              ...
              <div className="ml-auto flex items-center gap-2">
                {entry.data && hasCurrentResult && entry.ip !== currentIp && (
                  <Button
                    variant="ghost"
                    size="sm"
                    startIcon="compare"
